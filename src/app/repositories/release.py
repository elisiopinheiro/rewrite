...
_RELEASE_LOAD_OPTIONS: tuple[Any, ...] = (selectinload(Release.features).selectinload(ReleaseFeature.feature),)
...
class ReleaseRepository:
    ...
    def list_by_name_and_provider(self, *, name: str, provider: str) -> list[Release]:
        statement = (
            self._base_statement().where(Release.name == name, Release.provider == provider).order_by(asc(Release.id))
        )
        return list(self.session.scalars(statement).all())
    ...
    def add_from_data(self, data: dict[str, Any], features: list[Feature]) -> Release:
    ...
    @staticmethod
    def _base_statement() -> Select[tuple[Release]]:
